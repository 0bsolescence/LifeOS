...
- **The source sweep is one fan-out held by the orchestrating turn.** Dispatch parallel `Agent` calls, one per network-bound source; small local reads collapse into a single agent. Each source's brief lives in this file — it is the source of truth for what a source is and how it is fetched. Each delegate returns its findings and coverage status; the report is written by the orchestrating turn, since the output format is a voice contract a delegate does not hold.
- **Deadline is a ceiling:** ~4 minutes, fail-open, held by the orchestrating turn. A source that times out or dies comes back in `coverage` with that status and is reported as `⏳ timed out` in Sources Processed. Never re-fire a slow agent.
