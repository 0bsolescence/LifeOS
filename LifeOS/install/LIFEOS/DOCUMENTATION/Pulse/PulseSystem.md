...
| **Bunker** | Feeds the `/bunker` tab. Shells out to the Bunker CLI (`LIFEOS/PULSE/Bunker`, or `BUNKER_DIR`), which is the source of truth when present. That CLI is private upstream infrastructure and is not deployed on this install, so the module fail-softs: one info line at startup, no scans scheduled, `health()` reports `unavailable`, and the tab renders inert. The Arbol security/site-health/cost routes it also serves are independent of the CLI and keep working. | `modules/bunker.ts` |
