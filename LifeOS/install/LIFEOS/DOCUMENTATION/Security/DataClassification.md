...
Machine-readable form: `maxClassForRoute()` in `hooks/lib/data-classification.ts`. Live enforcement: `hooks/EgressClassGuard.hook.ts`. <!-- Corrected 2026-08-25 (local-rung research D-2): previously pointed at LIFEOS/TOOLS/models.ts, which holds Claude tier data only ("data, not policy"); a ROUTES table no longer exists. -->
...
- `hooks/lib/data-classification.ts` — `RESTRICTED_CAPABLE_VENDORS`, `CHINESE_MODEL_PATTERNS`, `maxClassForRoute()`. <!-- Corrected 2026-08-25: models.ts holds Claude tier data only; SOURCE_TIER, isRouteAllowed() and ROUTES no longer exist anywhere in the classification path (verified tree-wide) — references tombstoned rather than relocated. -->
