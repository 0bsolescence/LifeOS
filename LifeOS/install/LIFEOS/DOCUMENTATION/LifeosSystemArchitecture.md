...
| **Bunker** | **Concept adopted; implementation NOT deployed here (2026-08-20).** The reference implementation — `LIFEOS/PULSE/Bunker` with CLI `bin/bunker.ts` — is private upstream infrastructure and ships in no public release payload (upstream `DOCUMENTATION/Bunker/BunkerSystem.md` release note, verified against `danielmiessler/LifeOS` main 2026-08-20). It is absent on both nodes, so the server half is inert: `Pulse/modules/bunker.ts` logs one info line at startup, schedules no scan, reports `status: unavailable` on `/api/bunker/status`, and answers `/api/bunker` with `available: false`. **Open gap (2026-08-20):** the `/bunker` page itself has not been taught that field — it renders its empty board under a hardcoded green "SYSTEMS ONLINE" pill, which is cosmetically misleading on an install with no harness. Fixing it needs a Next.js static-export rebuild and was left out of the fail-soft change. Building our own implementation is a queued decision, not a plan. Superseded claim, kept as history: earlier revisions of this row said the code was "folded in, no separate repo" at `LIFEOS/PULSE/Bunker` (itself a 2026-07-29 correction of a stale `~/Projects/bunker` path, public PR #1627, @elhoim) — no such tree has ever existed on this estate. What DOES run here is the Security plane (below), which is independent of the CLI. The concept: a universal application harness — every app declares a `type` that composes cross-cutting components across six planes (data, control, observability, identity, security, commerce); **the Security plane IS the Arbol infra-security scanner** (server-side, hourly — the Arbol security system became Bunker's Security plane 2026-07-20; never build a parallel one, and every public deployment registers in BOTH the Bunker ISA and the scanner's curated inventory); the app's `bunker.isa.md` is simultaneously its spec, component manifest, executable test suite (`bunker test` runs the `## Test Strategy` probes), and **stored current state of the application** — "add a feature" = add claims to the ISA that don't hold yet. Upstream's private v0.1.0 spine (CLI `bin/bunker.ts`, `src/{adopt,config,isa,registry,test,admin}.ts`) is what the adopted apps samsaid, dariosaid, oursafe and shouldwecontrolopensource run — upstream's estate, not ours. The shipping half here is `Pulse/modules/bunker.ts` + the `/bunker` page, which render only when a CLI is present (`BUNKER_DIR` overridable). Design provenance: `MEMORY/WORK/20260708-chassis-application-harness/ISA.md` (E5), Pulse interface: `MEMORY/WORK/20260708-bunker-pulse-interface/ISA.md`. |
